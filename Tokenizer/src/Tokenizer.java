...
public final class Tokenizer {
    ...
    private static String nextWordOrSeparator(String text, int position) {
        ...
        int i = position;
        String result = "";
        boolean first = text.charAt(i) == SEPARATORS.charAt(0);
        while (i < text.length()
                && text.charAt(i) == SEPARATORS.charAt(0) == first) {
    ...
    public static Queue<String> tokens(SimpleReader in) {
        ...
        while (!in.atEOS()) {
            ...
            if (!text.equals("")) {
                int i = 0;
                while (i < text.length()) {
                    String s = nextWordOrSeparator(text, i);
                    if (!s.contains(SEPARATORS.subSequence(0, 1))) {
                        q.enqueue(s);
                    }
                    i += s.length();
                }

...
public abstract class ProgramTest {
    ...
    public final void testSwapContext2() {
        ...
        pRef.swapContext(contextRef);
        Pair<String, Statement> oneRef = contextRef.remove(oneName);
        /* contextRef now has just "TurnAround" */
        pRef.swapContext(contextRef);
        /* pRef's context now has just "TurnAround" */
        contextRef.add(oneRef.key(), oneRef.value());
        /* contextRef now has just "FindObstacle" */

        /*
         * Make the reference call, replacing, in pRef, "FindObstacle" with
         * "TurnAround":
         */
        pRef.swapContext(contextRef);
        ...
        pTest.swapContext(contextTest);
        Pair<String, Statement> oneTest = contextTest.remove(oneName);
        /* contextTest now has just "TurnAround" */
        pTest.swapContext(contextTest);
        /* pTest's context now has just "TurnAround" */
        contextTest.add(oneTest.key(), oneTest.value());
        /* contextTest now has just "FindObstacle" */
    ...
    public final void testSwapBody2() {
        ...
        Statement bodyTest = pTest.newBody();
        pRef.swapBody(bodyRef);
        Statement firstRef = bodyRef.removeFromBlock(1);
        /* bodyRef now lacks the second statement */
        pRef.swapBody(bodyRef);
        /* pRef's body now lacks the second statement */
        bodyRef.addToBlock(0, firstRef);
        /* bodyRef now has just the second statement */

        /* Make the reference call, replacing, in pRef, remaining with first: */
        pRef.swapBody(bodyRef);

        pTest.swapBody(bodyTest);
        Statement firstTest = bodyTest.removeFromBlock(1);
        /* bodyTest now lacks the second statement */
        pTest.swapBody(bodyTest);
        /* pTest's body now lacks the second statement */
        bodyTest.addToBlock(0, firstTest);
        /* bodyTest now has just the second statement */

    ...
    public void prettyPrint(SimpleWriter out, int offset) {
        ...
        switch (this.kind()) {
            ...
            case IF: {
                ...
                Condition c = this.disassembleIf(sub);
                printSpaces(out, offset);
                out.println("IF " + toStringCondition(c) + " THEN");
                sub.prettyPrint(out, offset + INDENT_SIZE);
            ...
            case IF_ELSE: {
                ...
                Condition c = this.disassembleIfElse(sub1, sub2);
                printSpaces(out, offset);
                out.println("IF " + toStringCondition(c) + " THEN");
                sub1.prettyPrint(out, offset + INDENT_SIZE);
                printSpaces(out, offset);
                out.println("ELSE");
                sub2.prettyPrint(out, offset + INDENT_SIZE);
                this.assembleIfElse(c, sub1, sub2);
                printSpaces(out, offset);
                out.println("END IF");
            ...
            case WHILE: {
                ...
                Statement sub = this.newInstance();
                Condition c = this.disassembleWhile(sub);
                printSpaces(out, offset);
